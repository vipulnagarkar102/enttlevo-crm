...
const UnassignedLeadsTable: React.FC = () => {
  ...
  const unassignedData = [
    { 
      id: 1, account: 'Mirrat', contact: 'Meghna', email: 'meghna@mirrat.com', industry: 'IT', product: 'enttevo', status: 'Touchbase', label: 'Hot', created: '01-12-2024', updated: '03-01-2025',
      address: 'Dehu road pune', city: 'Pune', state: 'Maharashtra', country: 'India', postal: '411033', size: '50-100', revenue: '$2M', website: 'mirrar.com', description: 'Interested in CRM expansion'
    },
  ...
  return (
    <div className="bg-white border border-outline/5 rounded-sm overflow-hidden shadow-sm flex flex-col relative">
        ...
        <table className="w-full text-left border-collapse min-w-[2000px]">
          <thead>
            <tr className="bg-white border-b border-outline/5">
              {isVisible('account_name') && <th className="px-6 py-4 text-[0.7rem] font-bold text-on-surface-variant/40 uppercase tracking-widest whitespace-nowrap sticky left-0 bg-white z-20 shadow-[2px_0_5px_rgba(0,0,0,0.05)]">Account Name</th>}
                ...
                >
                  ...
                  {isVisible('contact_name') && (
                    <td className="px-6 py-0">
                      <span className="text-[0.85rem] font-medium text-on-surface-variant">{lead.contact}</span>
                    </td>
                  )}
                  {isVisible('email') && (
                    <td className="px-6 py-0">
                  ...
                  {isVisible('industry') && (
                    <td className="px-6 py-0">
                      <span className="text-[0.8rem] text-on-surface-variant/80 font-medium uppercase tracking-tight">{lead.industry}</span>
                    </td>
                  )}
                  {isVisible('product_name') && (
                    <td className="px-6 py-0">
                      <span className="text-[0.8rem] text-on-surface font-semibold">{lead.product}</span>
                    </td>
                  )}
                  {isVisible('account_status_id') && (
                    <td className="px-6 py-0">
                      <span className={`inline-block text-center w-[110px] px-2 py-1 rounded-sm text-[0.65rem] font-bold border ${
                        ...
                        lead.status === 'Touchbase' ? 'bg-blue-50 text-blue-700 border-blue-100' :
                  ...
                  {isVisible('lead_source') && (
                    <td className="px-6 py-0">
                      <span className="text-[0.75rem] text-on-surface-variant font-medium">self</span>
                    </td>
                  )}
                  {isVisible('created_at') && (
                    <td className="px-6 py-0">
                      <span className="text-[0.75rem] text-on-surface-variant/60 font-medium">{lead.created}</span>
                    </td>
                  )}
                  {isVisible('updated_at') && (
                    <td className="px-6 py-0">
                      <span className="text-[0.75rem] text-on-surface-variant/60 font-medium">{lead.updated}</span>
                    </td>
                  )}
                  {isVisible('address') && (
                    <td className="px-6 py-0">
                      <span className="text-[0.8rem] text-on-surface-variant/70 italic">{lead.address || '-'}</span>
                    </td>
                  )}
                  {isVisible('city') && (
                    <td className="px-6 py-0">
                  ...
                  {isVisible('postal') && (
                    <td className="px-6 py-0">
                      <span className="text-[0.8rem] text-on-surface-variant/70">{lead.postal || '-'}</span>
                    </td>
                  )}
                  {isVisible('size') && (
                    <td className="px-6 py-0">
                      <span className="text-[0.8rem] text-on-surface font-medium">{lead.size || '-'}</span>
                    </td>
                  )}
                  {isVisible('revenue') && (
                    <td className="px-6 py-0">
                      <span className="text-[0.8rem] text-emerald-600 font-bold">{lead.revenue || '-'}</span>
                    </td>
                  )}
                  {isVisible('website') && (
                    <td className="px-6 py-0">
                  ...
                  {isVisible('description') && (
                    <td className="px-6 py-0">
                      <span className="text-[0.8rem] text-on-surface-variant/60 line-clamp-1 max-w-[200px]">{lead.description || '-'}</span>
                    </td>
                  )}
